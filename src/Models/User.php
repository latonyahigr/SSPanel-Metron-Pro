...
class User extends Model
{
    ...
    public function usedTrafficGB()
    {
        $total = $this->attributes['u'] + $this->attributes['d'];
        return  sprintf("%.2f",Tools::flowToGB($total));
    }
    ...
    public function unusedTrafficGB()
    {
        $total = $this->attributes['u'] + $this->attributes['d'];
        $transfer_enable = $this->attributes['transfer_enable'];
        return sprintf("%.2f",Tools::flowToGB($transfer_enable - $total));
    }
    /*
     * 剩余流量占总流量的百分比
     */
    ...
    public function TodayusedTrafficGB()
    {
        $total = $this->attributes['u'] + $this->attributes['d'] - $this->attributes['last_day_t'];
        return sprintf("%.2f",Tools::flowToGB($total));
    }
